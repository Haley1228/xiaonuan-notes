...
其中 σ 是 sigmoid 函数，⊙ 表示按元素相乘，方括号 \[F_v, F_l] 表示在通道维上的拼接（concatenate)
$F_v$视觉模型在每个时间步（字符位置）得到的特征或概率向量；T 为序列长度，C 为通道/特征维度$F_l$语言模型（BCN）在对应位置输出的语言特征或概率分布映射到相同维度后的向量

- \[Fv, Fl] ∈ R^{T×2C}：将视觉与语言特征在通道维拼接，从而让后续线性变换同时看到两者信息。
- **Wf ∈ R^{2C×C}：线性映射矩阵，把拼接后的 2C 维压回 C 维，得到每个位置的融合前激活。**
- **G ∈ R^{T×C}：通过 sigmoid 将激活映射成 0~1 的权重**，按位置和通道分别决定“信任视觉多少、信任语言多少”。
- Ff ∈ R^{T×C}：最终融合特征，逐元素为视觉和语言按 G 的权重加权平均。
### 迭代 
![[iterative.png]]

- Pm(yt)：在第 m 次迭代中，第 t 个字符的预测概率分布（向量）。  
- P(yt) = max_{1 ≤ m ≤ M} Pm(yt)：这里把 M 次迭代得到的概率取“最大值”——通常是指取每个字符类别上 M 次预测概率的最大值，最终得到一个更乐观/强置信度的分布 P(yt)。目的是在多次迭代中取到最强的信号，减少单次噪声影响。  
- E\[log P(yt)]：对字符位置 t 的概率分布取对数并求期望（即该位置的 log-probability 的期望），常用于衡量该位置的确定性（log 概率越大，越确定)
- C = min_{1 ≤ t ≤ T} e^{E[log P(yt)]}：对一个文本样本，计算所有字符位置 t 的 e^{E[log P(yt)]}（等价于按位置的几何平均概率或位置置信度），然后**取最小值作为该文本的整体置信度 C**
