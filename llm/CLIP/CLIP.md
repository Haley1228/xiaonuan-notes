# 0.背景知识及优势
## 1）对比学习是什么？

核心思想是让模型学习区分相似和不相似的样本对

1. 给定一个批次（batch）的 N对（图像，文本）数据。
2. 模型包含一个图像编码器和一个文本编码器，分别生成图像和文本的特征表示。
3. 将这些表示映射到一个共享的多模态嵌入空间。在这个空间中，目标是**最大化批次内真正匹配的（图像，文本）对的相似度**（例如，通过计算余弦相似度），同时**最小化批次内所有不匹配的（图像，文本）对的相似度**。
4. 作者提到，这种方法在零样本（zero-shot）迁移任务上的效率比直接预测文本的方法提高了 4 倍 。

## 2）优势

- 传统方法：依赖于这种固定的、预设的分类体系，例如 ImageNet 中的 1000 个类别。*这限制了模型的泛化能力*，因为要识别新的概念，就必须重新标注数据并重新训练模型。
- 利用了**图像旁边存在的自然语言描述** 。这些文本本身就包含了丰富的视觉概念信息，并且不需要转换为预设的“1-of-N”格式.
- 这使得模型能够学习更广泛、更灵活的视觉概念，并且能够通过简单的文本提示（如“一张{类别名称}的照片”）来实现零样本迁移，而无需针对新任务进行任何数据标注或模型重新训练。




# 1.流程图

![公式](通往未来/CLIP/image/1.png)
图中左侧矩阵，正样本就是对角线上的匹配数据对$n$,负样本就是$n^2-n$个


![公式](1_.png)

这张图展示了 CLIP（Contrastive Language-Image Pre-training）模型的核心思想和工作流程，主要分为两个阶段：**对比预训练 (Contrastive Pre-training)** 和 **零样本预测 (Zero-shot Prediction)**
1. 对比预训练
模型接收大量的（图像，文本）配对数据。模型包括两个主要的编码器，图像编码器接收图像并将其转换为一个向量表示，文本编码器接收文本并将其转换为一个向量表示

核心目标是让模型学会预测哪个图像与哪个文本描述是匹配的。在一个批次（batch）中，假设有N对（图像，文本）数据。模型会计算出 N x N 种可能的图像-文本相似度分数 其中对角线上的配对样本代表真实的配对，而非对角线上的代表错误的配对。

通过对比学习（Contrastive Learning），模型被训练来最大化真实配对的相似度，同时最小化错误配对的相似度。**这使得模型学习到一个统一的多模态嵌入空间，能够将语义相关的图像和文本映射到相近的位置**。

2. 零样本预测

在模型预训练完成后，我们可以用它来对新的、未见过的数据进行分类，而无需进一步的训练（即零样本迁移）

**文本生成分类器**
- 首先，定义一组潜在的类别（例如，“plane”, “car”, “dog”, “bird”）。
- 这些类别名称会通过一个模板（例如，“A photo of a {object}.”）进行加工，转化为更具描述性的文本（例如，“A photo of a dog.”）。
- 这些加工后的文本被送入文本编码器 (Text Encoder)，为每个类别生成一个文本嵌入向量 ($T_1, T_2, ..., T_N$​)。这些文本嵌入向量可以看作是模型创建的“零样本线性分类器”的权重。
**图像分类**
- 一张待分类的图像（例如，一只黑白相间的小狗）被送入图像编码器 (Image Encoder)，生成其图像嵌入向量 。
- 然后，计算该图像嵌入向量与所有预先生成的文本类别嵌入向量之间的相似度（例如，$I_1 \cdot T_1, I_1 \cdot T_2, I_1 \cdot T_3, ..., I_1 \cdot T_N$）。
- 模型预测得分最高的文本描述所对应的类别，即为该图像的预测结果。

总而言之，CLIP通过在一个大规模数据集上进行图像-文本配对的对比预训练，学习到一个强大的==视觉-语言关联能力==。在推理时，它能够利用这种能力，通过将图像和文本描述（通过自然语言模板生成）嵌入到同一空间，从而实现对新图像的零样本分类。

# 2.伪代码解析

![公式|500](通往未来/CLIP/image/3.png)
## 1）输入

**图像编码器**: 它接收批次中的所有 N 张图像，并为每张图像生成一个向量表示（图像嵌入）。我们把这 N 个图像嵌入放在一个矩阵里，称为 $I_e$，它的形状大概是 ``[N, embedding_dim]``
**文本编码器**: 它接收批次中的所有 N 段文本，并为每段文本生成一个向量表示（文本嵌入）。我们把这 N 个文本嵌入放在一个矩阵里，称为 $T_e$，它的形状大概是`` [N, embedding_dim]``

## 2）损失函数设计

![公式|500](3notes.jpg)

# 3.代码解释

### a. Bottleneck

标准残差瓶颈块

## b.🌟AttentionPool2d

输入 x:\[N,C,H,W]
输出 \[N,output_dim]

和普通平均池化相比：
- 平均池化：所有位置一视同仁
- 注意力池化：不同位置权重可以不同，模型自己学

**不是简单把整张特征图求平均，而是让一个“全局 token”去关注所有空间位置，再汇总出整图向量**
## <1>关于图像的处理

1. 图像预处理
-  `Resize(n_px)`：缩放到模型需要的尺寸
- `CenterCrop(n_px)`：中心裁剪成正方形
- `convert("RGB")`：统一成三通道
- `ToTensor()`：变成张量
- `Normalize(...)`：按 CLIP 训练时的均值方差归一化

所以模型真正接收到的图像，一般是这种形状：
- `[batch, 3, H, W]`
比如常见的是 `[1, 3, 224, 224]`。
2. **图像编码器**
**ModifiedResNet**
- 前面几层卷积先提取低层视觉特征
- 后面 `layer1 ~ layer4` 逐步提取更高层语义
- 最后不是普通平均池化，而是 `attnpool`[[#b.🌟AttentionPool2d]]

**卷积提局部特征  残差层提层级语义  注意力池化做全局汇总**

**VisionTransformer**
1. 
- 这里的卷积核大小和步长都等于 `patch_size`。  
所以它的作用基本就是：
- 把整张图切成互不重叠的小块
- 每个小块映射成一个向量
2. 
把二维 patch 网格改成一串 token：
- 从图像张量
- 变成 patch 序列
也就是把“图”转成“序列”，这样才能送进 Transformer，前面加一个专门的**全局 token**，负责最后汇总整张图的信息。
3.  加位置编码
4. 送入transformer
这一步让每个 patch 可以和其他 patch 全局交互。  
这是 ViT 和 CNN 最大的区别之一：
- CNN 更偏局部、逐层扩大感受野
- ViT 一开始就能做全局关系建模
5. 取第一个token
最后 `x[:, 0, :]` 就是 class token 的输出，代表整张图的语义。
6. 
乘 `proj`，再把它投影到 CLIP 的共享嵌入空间，方便后面和文本向量比较。

**先把图片标准化，再用 ResNet 或 ViT 抽取高层语义，最后把整张图变成一个可以和文本直接比较的向量。**

## <2>关于文本的处理

### 1.文本变成token

- 如果输入是单个字符串，先包装成列表
- 在句子前后加特殊标记：
    - `<|startoftext|>`
    - `<|endoftext|>`
- 调用 `_tokenizer.encode(text)` 做真正分词
- 补齐到固定长度 `77`
- 太长就截断或报错

所以最后拿到的是一个二维整数张量，形状像：
- `[batch_size, 77]`
比如两句话就可能是：
- `[2, 77]`

之后对文本进行清洗和分词 **采用BPE合并规则切成字词**，最终映射成词表里的整数id
### 2.embedding

一句话中的每个token，变成一个稠密语义向量
例如：
- 输入：`[batch_size, 77]`
- 输出：`[batch_size, 77, transformer_width]`

### 3.位置编码

### 4. Text Transformer

#### CLIP与BERT的区别

**CLIP 的文本侧做法是：**
- 输入句子前后加特殊 token
- 用因果 mask，也就是每个位置只能看前面
- 最后取 `eot` 位置的输出作为整句表示
优点：
- 设计很统一，和 GPT 风格一致
- `eot` 天然就是“读完了”的位置
- 不需要额外引入 `[CLS]` 作为专门汇总位
缺点：
- **中间词的表示看不到后文**
- 不是双向上下文
- **更适合“最后汇总整句”，不一定最适合做细粒度 token 理解**

**BERT  `[CLS] + bidirectional attention`**
- 在句首加一个特殊 token：`[CLS]`
- 用 **双向注意力**
- 最后取 `[CLS]` 的输出作为整句表示

**专门放一个“句子代表位”在最前面，让它和整句所有词双向交互，最后它负责汇总整句信息。**

优点:
- 每个 token 都能看左右上下文
- `[CLS]` 可以从一开始就被设计成“句子级表示位”
- 更适合很多 NLU 任务，比如分类、匹配、推断
缺点：
- 需要额外定义 `[CLS]` 的角色
### 5.乘投影矩阵 到文本图像共享空间

## 文本图像对齐

把图像向量和文本向量都放到同一个向量空间里，再算余弦相似度，每张图和每句文本两两做点积。



