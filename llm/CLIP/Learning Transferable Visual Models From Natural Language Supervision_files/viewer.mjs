...
const BUTTON_SCALE_DELTA = 1.1; // +/- 버튼용 배율 
const WHEEL_SCALE_DELTA = 1.01;  // 마우스 휠용 배율
const MIN_SCALE = 0.1;
  ...
  static #enableGlobalSelectionListener() {
    ...
    document.addEventListener("selectionchange", () => {
      ...
      if (endDiv) {
        ...
        if (anchor.classList.contains("highlight") || anchor.classList.contains("comment-source-xpath")) {
          parentTextLayer.insertBefore(endDiv, modifyStart ? anchor.closest("span") : anchor.closest("span").nextSibling);
        } else {
...
class PDFPageView {
  ...
  reset({
    ...
    for (let i = childNodes.length - 1; i >= 0; i--) {
      ...
      if (node.classList.contains('translate-page-wrapper')) {
        continue;
      }
      node.remove();
...
class PDFViewer {
  ...
  setDocument(pdfDocument) {
    ...
    this._pagesCapability.promise.then(() => {
      ...
      window.top.postMessage({ type: 'pdfLoaded' }, '*');
    }, () => { });
  ...
  updateScale({
    ...
    if (scaleFactor > 0 && scaleFactor !== 1) {
      ...
      const delta = steps > 0 ? scaleDelta : 1 / scaleDelta;
      const round = steps > 0 ? Math.ceil : Math.floor;
      steps = Math.abs(steps);
      do {
        newScale = round((newScale * delta).toFixed(3) * 100) / 100; // .tooFixed(3) -> 1% 단위로 조절
      } while (--steps > 0);
    }
...
const PDFViewerApplication = {
  ...
  updateZoom(steps, scaleFactor, origin, source) {
    if (this.pdfViewer.isInPresentationMode) {
      return;
    }
    const scaleDelta = source === 'button' ? BUTTON_SCALE_DELTA : WHEEL_SCALE_DELTA;
  ...
  zoomIn() {
    this.updateZoom(1, null, null, 'button');
  },
  zoomOut() {
    this.updateZoom(-1, null, null, 'button');
  },
...
function webViewerWheel(evt) {
  ...
  if (isPinchToZoom || evt.ctrlKey && supportsMouseWheelZoomCtrlKey || evt.metaKey && supportsMouseWheelZoomMetaKey) {
    ...
    if (isPinchToZoom && supportsPinchToZoom) {
      ...
      PDFViewerApplication.updateZoom(null, scaleFactor, origin, 'wheel');
    } else {
      ...
      PDFViewerApplication.updateZoom(ticks, null, origin, 'wheel');
    }
  }
}
...
function webViewerTouchMove(evt) {
  ...
  if (supportsPinchToZoom) {
    ...
    PDFViewerApplication.updateZoom(null, newScaleFactor, origin, 'wheel');
  } else {
    const PIXELS_PER_LINE_SCALE = 30;
