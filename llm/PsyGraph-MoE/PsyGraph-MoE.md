...
## over-smoothing
在图神经网络中，当图层数太深或传播次数太多时，图中所有节点的特征向量会变得越来越像，最后几乎完全一样
- 失去细粒度判别力
- 特征异质性丧失
...
- 冗余信息损失 (Redundancy Loss)：提取所有视角中都存在的重复信息
    通过最小化全视角与两个单模态中心视角之间的预测差异，**迫使 Red 专家学习那些在音频和视觉中表现一致的语义特征**
- 协同信息损失 (Synergy Loss)：提取仅在多模态融合时才会产生的“1+1>2”的新语义
    它要求全视角0与任何单模态视角1或2之间的差异必须大于m，如果差异不够大，损失就会增加。**这迫使 Syn 专家不去学习单一模态就能提供的特征，而是专门捕获那些必须结合视听线索才能理解的复杂情感特征**
- 音频/视觉特有损失 (Uniqueness Hinge Loss)：**强制专家只关注音视频独有的信息**
    这是一个铰链损失（Hinge Loss）它要求全视角与音频视角 的预测差异尽可能小，而全视角 与视觉视角 的预测差异尽可能大（至少比前者大出距离m）
# Experiment
![](image/ablation.png)
- 基础性能 (第一行)：仅使用原始模态（Text, Audio, Video）时，5类准确率50.29，7类准确率44.75
- 线索的有效性 (第二行)：简单加入非语言线索后，性能提升至 52.08 / 47.13，证明了细粒度线索包含丰富的情感信息
>非语言线索：
>1 视觉线索：面部动作单元 (Action Units, AUs)
 面部肌肉的微小动作。论文采用了 FACS（面部动作编码系统） 标准，将表情拆解为具体的动作：
 AU01 (抬起内眉)：通常与忧虑或悲伤相关。
 AU12 (拉紧嘴角)：即微笑，通常与快乐相关。
 AU45 (眨眼)：频率变化可能反映紧张或思考。
  2 听觉线索：声学韵律 (Acoustic Prosody)
不是指说话的文字，而是声音的“质感”和“节奏”。
 例子：
 音高 (Pitch)：说话变尖（高音）可能代表兴奋或愤怒。
 响度 (Loudness)：声音大小。
 抖动 (Jitter) 和 闪烁 (Shimmer)：声音的频率和振幅波动，通常用来捕捉“嗓音颤抖”等细微的情绪不稳定感
 
- 图传播的作用 (第三行)：在加入 HDAP 后，准确率显著提升至 54.60 / 49.58。**这验证了通过图结构捕捉线索间的“共现模式”能有效缓解“扁平化线索坍塌”问题，压制噪声并强化关键信号**
- 完整模型的最优性 (第四行)：当三个组件全部开启时，模型达到最优性能 (56.41 / 50.87)。**这说明解耦模块能够基于结构化后的特征，选择最具有信息量的因子进行增强**
- 关键的相互依赖 (第五行)：这是一个重要的对比——如果不使用 HDAP 却直接使用解耦模块，性能会大幅下降 (48.69 / 43.88)，甚至低于仅使用原始模态的效果。