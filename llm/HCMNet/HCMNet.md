...
1. **跨粒度关联的层次化建模问题 (Hierarchical Correlation Modeling)这是你提到的“双粒度结合”。作者认为不仅要看局部，还要看全局结构。**
- 原子级 (Atomic)：解决如何利用动态注意力机制在相同的语义空间内对齐独立的帧、词和音频段 
- 组合级 (Compositional)：解决如何通过拓扑图捕捉模态内部的依赖关系（如词法依赖、视频空间依赖），从而挖掘隐性的“弱相关”信号
2. **跨模态语义分布的不一致性问题 (Distributional Inconsistency)这是该论文区别于普通融合方法的关键。即使两个模态都在表达同一种情绪，它们的特征分布也可能完全不同（见论文图 1）**。
- 原子级分布约束：如何利用对比学习约束不同模态的特征空间，使得相同情感的特征被拉近，不同情感的被推开，从而减少原始特征融合时的歧义性
- 组合级分布约束：如何解决复杂图结构之间的对齐问题？作者引入了最优传输  理论，通过寻找最小成本的传输流 ，将一个模态的图语义自动映射/对齐到另一个模态的图语义上
